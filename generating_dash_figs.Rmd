---
title: "generating maps for the dashboard"
...
generate_and_save_map <- function(selected_year, file_name) {
  filtered_data <- ny_data %>% filter(year == selected_year)
  leaflet_map <- leaflet(data = filtered_data) %>%
    addTiles() %>%
    addPolygons(
      fillColor = ~colorNumeric("YlOrRd", lyme_count)(lyme_count),
      fillOpacity = 0.7,
      color = "black",
      weight = 1,
      popup = ~paste(
    ...
    addLegend(
      position = "topright",
      pal = colorNumeric("YlOrRd", filtered_data$lyme_count),
      values = filtered_data$lyme_count,
      title = "Lyme Count"
    )
  htmlwidgets::saveWidget(leaflet_map, file_name)
}
...
```{r}
generate_and_save_map(2019, "lyme_map_2019.html")
generate_and_save_map(2020, "lyme_map_2020.html")
generate_and_save_map(2021, "lyme_map_2021.html")
generate_and_save_map(2022, "lyme_map_2022.html")
```
